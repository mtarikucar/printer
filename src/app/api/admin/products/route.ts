...
export async function GET(request: NextRequest) {
  try {
    ...
    return handleRouteFailure(e, "GET /api/admin/products", ADMIN_READ_FAILED_ERROR);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: "Ürün oluşturulamadı. Ürünün kaydedilip kaydedilmediğini görmek için listeyi yenileyin; sorun sürerse sunucu günlüklerine bakın." }, { status: 500 });
    }
  } catch (e) {
    return handleRouteFailure(e, "POST /api/admin/products", ADMIN_ACTION_FAILED_ERROR);
  }
}
