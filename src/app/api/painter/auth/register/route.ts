...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: "Kayıt tamamlanamadı. Kaydınızın oluşup oluşmadığını görmek için giriş yapmayı deneyin; sorun sürerse bizimle iletişime geçin." }, { status: 500 });
    }
  } catch (e) {
    return handleRouteFailure(e, "POST /api/painter/auth/register", PARTNER_ACTION_FAILED_ERROR);
  }
}
