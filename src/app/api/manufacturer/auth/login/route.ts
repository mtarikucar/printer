...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: "Giriş yapılamadı. Birkaç saniye sonra tekrar deneyin; sorun sürerse bizimle iletişime geçin." }, { status: 500 });
    }
  } catch (e) {
    return handleRouteFailure(e, "POST /api/manufacturer/auth/login", PARTNER_ACTION_FAILED_ERROR);
  }
}
