...
/**
 * Turun fotoğraf SAYISI okunamadığında boyacıya dönen cümle.
 *
 * Sayım, tur başına 6 fotoğraf kapısının girdisidir ve kapının ÖNÜNDE okunur.
 * Korumasız hâlinde boyacı dosyaları gönderdikten sonra rota burada fırlıyor,
 * panele sebebi olmayan genel bir 500 düşüyordu; üstelik iş listesi ekranı bu
 * arızada yükleyiciyi sunmaya devam ediyor (painter/jobs/jobs-client.tsx).
 * Ekranın sunduğu kontrolün, arızada da CEVAP veren bir ucu olmalı.
 */
const QC_PHOTO_COUNT_UNAVAILABLE_ERROR =
  "Bu turda kaç QC fotoğrafı olduğu şu anda okunamadı (geçici sistem arızası). Tur başına 6 fotoğraf sınırı bilinmeyen bir sayıya uygulanamayacağı için yükleme yapılmadı. Daha önce yüklediğiniz fotoğraflar SİLİNMEDİ — lütfen yeniden yüklemeyin, birkaç dakika sonra tekrar deneyin.";
...
export async function POST(
  ...
  try {
    ...
    // TUR BAŞINA SAYIM: 6 fotoğraflık kapının tek girdisi — üretici ikizindeki
    // ile aynı korumayı taşır. Kapı KAPALI kalır (fail closed): bilinmeyen bir
    // sayıya 6 sınırı uygulanamaz, yani bu dal fazladan fotoğraf yazdırmaz.
    let existing: number;
    try {
      ...
      console.error("painter qc-photos: tur başına QC fotoğraf sayısı okunamadı", e);
      return NextResponse.json(
    ...
    return handleRouteFailure(e, "POST /api/painter/orders/[id]/qc-photos", PARTNER_ACTION_FAILED_ERROR);
  }
}
